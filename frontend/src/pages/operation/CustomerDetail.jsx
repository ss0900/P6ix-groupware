...
import { FiArrowLeft, FiEdit2, FiTrash2, FiPlus, FiPhone, FiMail, FiUser } from "react-icons/fi";
...
function CustomerDetail() {
  ...
  const [newContact, setNewContact] = useState({
    name: "", position: "", department: "", email: "", phone: "", mobile: "", is_primary: false
  });
  ...
  const handleAddContact = async (e) => {
    ...
    try {
      await CustomerService.createContact({ ...newContact, company: parseInt(id) });
      setContactModal(false);
      setNewContact({ name: "", position: "", department: "", email: "", phone: "", mobile: "", is_primary: false });
      fetchCompany();
    } catch (error) {
  ...
  const handleDelete = async () => {
    if (!window.confirm("정말 삭제하시겠습니까? 관련된 담당자 정보도 삭제됩니다.")) return;
    try {
  ...
  if (!company) {
    return <div className="text-center py-12 text-gray-500">고객사를 찾을 수 없습니다.</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate("/operation/customers")} className="p-2 hover:bg-gray-100 rounded-lg">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => {}} className="btn-edit flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="page-box">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setContactModal(true)} className="btn-create-sm flex items-center gap-1">
                ...
                <div key={contact.id} className="p-3 border border-gray-200 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {contact.is_primary && (
                        <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs rounded">주담당</span>
                      )}
                    </div>
                    {contact.position && (
        ...
        {/* 우측: 관련 영업기회 */}
        <div className="page-box">
          <h3 className="text-sm font-semibold text-gray-700 mb-4">관련 영업기회</h3>
          
          {leads.length === 0 ? (
            <p className="text-center text-gray-500 py-8">관련 영업기회가 없습니다.</p>
          ) : (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`text-xs ${
                      ...
                      lead.status === "lost" ? "text-red-600" : "text-gray-500"
                    }`}>
      ...
      {/* 담당자 추가 모달 */}
      <Modal isOpen={contactModal} onClose={() => setContactModal(false)} title="담당자 추가">
        <form onSubmit={handleAddContact} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="is_primary" className="text-sm text-gray-700">주 담당자로 지정</label>
          </div>
          <div className="flex justify-end gap-2 pt-4">
            <button type="button" onClick={() => setContactModal(false)} className="btn-cancel">
              취소
            </button>
